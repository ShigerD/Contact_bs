...
public class HomeContactActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences sharedPreferences = getSharedPreferences("mycontact",
				Context.MODE_PRIVATE);
		// getString()第二个参数为缺省值，如果preference中不存在该key，将返回缺省值
		isAddFormContact = sharedPreferences.getBoolean("isAddFormContact",
				false);
		...
		alpha = (QuickAlphabeticBar) this.acbuwaPage
		...
		menuBtn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				// 添加联系人
				TopPopup topPopup = new TopPopup(HomeContactActivity.this,
						handler);
				topPopup.setItems(new String[] { "从手机通讯录添加", "从SIM卡添加",
						"恢复联系人信息" });
				...
				if (backLists != null && backLists.size() > 0) {
					for (int i = 0; i < backLists.size(); i++) {
						for (int j = 0; j < list.size(); j++) {
							if (backLists.get(i).getLxid()
									.equals(list.get(j).getContactId() + "")) {
								list.remove(j);
							}
						}
					}
	...
	private String[] lianxiren1 = new String[] { "发送短信", "查看详细", "删除", "加入黑名单",
			"拨打电话", "快速拨号设置" };
		...
		new AlertDialog.Builder(this).setTitle(cb.getDisplayName())
				.setItems(arg, new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						...
						/*
						 * case 0://打电话 String toPhone = cb.getPhoneNum(); uri =
						 * Uri.parse("tel:" + toPhone); Intent it = new
						 * Intent(Intent.ACTION_CALL, uri); startActivity(it);
						 * break;
						 */
						...
						case 4:
							Intent intent = new Intent(Intent.ACTION_CALL, Uri
									.parse("tel:" + cb.getPhoneNum()));
							startActivity(intent);
							break;
						case 5:
							showJianPan(cb.getPhoneNum());
							break;
						}
					}
				}).show();
...
class MyOnclick implements OnClickListener{
				...
				.setPositiveButton("确定", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int whichButton) {
						...
						Uri lookupUri = ContactsContract.Contacts.getLookupUri(
								HomeContactActivity.this.getContentResolver(),
								deleteUri);
						...
						Toast.makeText(HomeContactActivity.this, "该联系人已经被删除.",
	...
	private void queryGroupMember(GroupBean gb) {
		...
		Cursor cur = getContentResolver().query(
				...
				ContactsContract.Data.MIMETYPE + " = '"
						...
						+ ContactsContract.Data.DATA1 + "=" + gb.getId(), null,
				"data1 asc");

		StringBuilder inSelectionBff = new StringBuilder()
				.append(ContactsContract.RawContacts._ID + " IN ( 0");
	...
	public void deleteBack() {
		try {
			...
			for (int i = 0; i < backLists.size(); i++) {
				if (backLists.get(i).getSmsid().equals("0")
						|| backLists.get(i).getSmsid().equals("1")) {
