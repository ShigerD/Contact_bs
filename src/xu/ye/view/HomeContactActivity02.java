...
import xu.ye.view.adapter.ContactHomeAdapter02;
...
public class HomeContactActivity02 extends Activity {
	...
	private ContactHomeAdapter02 adapter;
	...
	private   boolean isAddFormContact=false; //是否从通讯录添加
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ll_btn=(LinearLayout)acbuwaPage.findViewById(R.id.ll_btn);
		ll_btn.setVisibility(View.VISIBLE);
		
		SharedPreferences sharedPreferences2=getSharedPreferences("duanxinlist",Context.MODE_PRIVATE);
		final Editor editor=sharedPreferences2.edit();
		//确定button
		Button btn_confirm=(Button)acbuwaPage.findViewById(R.id.btn_confirm);
		btn_confirm.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				for(int i=0;i<numberList.size();i++){
					editor.putString("duanxin"+i, numberList.get(i));
				}
				editor.putInt("duanxin", numberList.size());
				Toast.makeText(HomeContactActivity02.this, "自定义拦截设置成功", 2000).show();
				HomeContactActivity02.this.finish();
				editor.commit();
			}
		});
		Button btn_cancle=(Button)acbuwaPage.findViewById(R.id.btn_cancle);
		...
		menuBtn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				/*//添加联系人
				TopPopup topPopup=new TopPopup(HomeContactActivity01.this);
				topPopup.setItems(new String []{"从手机通讯录添加","从手机SD卡添加","从SIM卡添加"});
				topPopup.show(menuBtn);*/
			}
		});
...
//					}
	...
	private void setAdapter(List<ContactBean> list,List<String> numberList) {	
		adapter = new ContactHomeAdapter02(this, list,numberList, alpha);
		personList.setAdapter(adapter);
		alpha.init(HomeContactActivity02.this);
		...
		new AlertDialog.Builder(this).setTitle(cb.getDisplayName()).setItems(arg,
			...
			public void onClick(DialogInterface dialog, int which){
				...
				case 1://发短息
					...
					BaseIntentUtil.intentSysDefault(HomeContactActivity02.this, MessageBoxList.class, map);
					break;
	...
	private void showDelete(final int contactsID, final int position) {
		...
		.setPositiveButton("确定", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				Uri lookupUri = ContactsContract.Contacts.getLookupUri(HomeContactActivity02.this.getContentResolver(), deleteUri);
				if(lookupUri != Uri.EMPTY){
					HomeContactActivity02.this.getContentResolver().delete(deleteUri, null, null);
				}
				adapter.remove(position);
				adapter.notifyDataSetChanged();
				Toast.makeText(HomeContactActivity02.this, "该联系人已经被删除.", Toast.LENGTH_SHORT).show();
			}
		})
...
//				}
	...
	private HomeContactActivity02.BaseReceiver1 receiver1 = null;
	/**
	 * 打开接收器
	 */
	private void startReceiver1() {
		if(null==receiver1){
			...
			receiver1 = new HomeContactActivity02.BaseReceiver1();
