...
import xu.ye.view.adapter.ContactHomeAdapter01;
...
public class HomeContactActivity01 extends Activity {
	...
	private ContactHomeAdapter01 adapter;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences sharedPreferences2=getSharedPreferences("numberlist",Context.MODE_PRIVATE);
		...
		btn_confirm.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				for(int i=0;i<numberList.size();i++){
					editor.putString("number"+i, numberList.get(i));
				}
				editor.putInt("number", numberList.size());
				Toast.makeText(HomeContactActivity01.this, "自定义拦截设置成功", 2000).show();
				HomeContactActivity01.this.finish();
				editor.commit();
			}
		});
...
//					}
	...
	private void setAdapter(List<ContactBean> list,List<String> numberList) {	
		adapter = new ContactHomeAdapter01(this, list,numberList, alpha);
		personList.setAdapter(adapter);
		alpha.init(HomeContactActivity01.this);
		...
		new AlertDialog.Builder(this).setTitle(cb.getDisplayName()).setItems(arg,
			...
			public void onClick(DialogInterface dialog, int which){
				...
				case 1://发短息
					...
					BaseIntentUtil.intentSysDefault(HomeContactActivity01.this, MessageBoxList.class, map);
					break;
	...
	private void showDelete(final int contactsID, final int position) {
		...
		.setPositiveButton("确定", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				Uri lookupUri = ContactsContract.Contacts.getLookupUri(HomeContactActivity01.this.getContentResolver(), deleteUri);
				if(lookupUri != Uri.EMPTY){
					HomeContactActivity01.this.getContentResolver().delete(deleteUri, null, null);
				}
				adapter.remove(position);
				adapter.notifyDataSetChanged();
				Toast.makeText(HomeContactActivity01.this, "该联系人已经被删除.", Toast.LENGTH_SHORT).show();
			}
		})
...
//				}
	...
	private HomeContactActivity01.BaseReceiver1 receiver1 = null;
	/**
	 * 打开接收器
	 */
	private void startReceiver1() {
		if(null==receiver1){
			...
			receiver1 = new HomeContactActivity01.BaseReceiver1();
