	...
	private class VCardScanThread extends Thread implements OnCancelListener,
		...
		public void run() {
			...
			try {
				...
				getVCardFileFormRoot(mRootDirectory);		
 ...
 * @throws IOException
 */
		private void getVCardFileFormRoot(File directory)
				throws CanceledException, IOException {
			if (mCanceled) {
				throw new CanceledException();
			}
           if(directory!=null&&directory.listFiles()!=null){
        	 ...
        	 String canonicalPath = file.getCanonicalPath();
				...
				mCheckedPaths.add(canonicalPath);
			 if (file.isFile()&&canonicalPath.toLowerCase().endsWith(".vcf")
						&& file.canRead()) {
					String fileName = file.getName();
					VCardFile vcardFile = new VCardFile(fileName,
							canonicalPath, file.lastModified());
	...
	protected void onStop() {
		...
		HomeContactActivity.init();//载入通讯录
		Toast.makeText(ImportVCardActivity.this, "从SD卡中添加成功！", 2000).show();
	...
	public void startImportVCardFromSdCard() {
		...
		if (!file.exists() || !file.isDirectory() || !file.canRead()) {
			...
			File sdcardDirectory = new File(Path.ROOTPATH);
